...
function run_ecg_batch_processing()
    ...
    for p = 1:length(Patient_Dirs)
        ...
        [Feature_Master, sig_count, leads_count] = process_patient_folder(patient_path, Patient_Dirs{p}, Feature_Master);
...
function [Feature_Master, signal_counter, lead_count_total] = process_patient_folder(patient_path, patient_id, Feature_Master)
...
if ~success
continue; % Skip this lead if reading fails
end
...
plot_normalization_comparison(t, ecg_filtered, ecg_norm, [signal_name ' - ' lead_names{lead_idx}]);
...
% Build row
row = table({patient_id}, {signal_name}, lead_idx, {lead_names{lead_idx}}, features.BPM, features.SDNN, ...
features.EnTotal, features.EnD5, features.Hurst, features.Higuchi, features.Katz, ...
'VariableNames', {'Patient_ID','Signal_Name','Lead_Index','Lead_Name','BPM_avg','SDNN','En_Total','En_D5','Hurst_Exp','Higuchi_FD','Katz_FD'});
